...
app.post('/captions', async (req, res) => {
  ...
  const result = await pool.query('INSERT INTO captions (topic, status) VALUES ($1, $2) RETURNING *', [topic, 'pending']);
...
app.patch('/captions/:id', async (req, res) => {
  ...
  const result = await pool.query('UPDATE captions SET caption = COALESCE($1, caption), status = COALESCE($2, status) WHERE id = $3 RETURNING *', [caption, status, id]);
